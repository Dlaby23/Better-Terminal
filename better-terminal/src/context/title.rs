...
pub fn create_title_extra_from_context<T: better_terminal::event::EventListener>(
    context: &Context<T>,
) -> Option<ContextTitleExtra> {
    #[cfg(unix)]
    let path =
        better_pty::foreground_process_path(*context.main_fd, context.shell_pid)
    ...
    #[cfg(unix)]
    let program =
        better_pty::foreground_process_name(*context.main_fd, context.shell_pid);
...
#[inline]
pub fn update_title<T: better_terminal::event::EventListener>(
    template: &str,
    context: &Context<T>,
) -> String {
        ...
        for (i, scoped_variable) in variables.iter().enumerate() {
            ...
            match var.as_str() {
                ...
                "program" => {
                    #[cfg(unix)]
                    {
                        let program = better_pty::foreground_process_name(
                            *context.main_fd,
                            context.shell_pid,
                        );
                ...
                "absolute_path" => {
                    ...
                    #[cfg(unix)]
                    {
                        let path = better_pty::foreground_process_path(
                            *context.main_fd,
                            context.shell_pid,
                        )
                        .map(|p| p.to_string_lossy().to_string())
...
pub mod test {
    ...
    use better_terminal::event::VoidListener;
    use better_terminal::better_gpu::layout::SugarDimensions;
