...
use better_terminal::config::Shell;
use smallvec::{smallvec, SmallVec};

use better_terminal::crosswords::{Crosswords, MIN_COLUMNS, MIN_LINES};
use better_terminal::error::{RioError, RioErrorLevel, RioErrorType};
use better_terminal::event::EventListener;
use better_terminal::event::WindowId;
use better_terminal::selection::SelectionRange;
use better_terminal::better_gpu::{font::SugarloafFont, Object, SugarloafErrors};
use std::borrow::Cow;
use std::error::Error;
...
use better_pty::create_pty;
#[cfg(not(target_os = "windows"))]
use better_pty::{create_pty_with_fork, create_pty_with_spawn};
...
pub struct Context<T: EventListener> {
    ...
    pub ime: Ime,
    _io_thread: Option<JoinHandle<(Machine<better_pty::Pty, T>, performer::State)>>,
}

impl<T: better_terminal::event::EventListener> Drop for Context<T> {
    fn drop(&mut self) {
        // Shutdown the terminal's PTY.
        let _ = self.messenger.channel.send(Msg::Shutdown);

        #[cfg(not(target_os = "windows"))]
        better_pty::kill_pid(self.shell_pid as i32);
    }
}
...
pub struct ContextManagerConfig {
    ...
    pub title: better_terminal::config::title::Title,
    pub keyboard: better_terminal::config::keyboard::Keyboard,
}
...
pub fn create_dead_context<T: better_terminal::event::EventListener>(
    event_proxy: T,
    window_id: WindowId,
    route_id: usize,
    rich_text_id: usize,
...
pub fn create_mock_context<
    T: better_terminal::event::EventListener + Clone + std::marker::Send + 'static,
>(
    event_proxy: T,
    window_id: WindowId,
    rich_text_id: usize,
...
impl<T: EventListener + Clone + std::marker::Send + 'static> ContextManager<T> {
    ...
    fn create_context(
        ...
        {
            if config.use_fork {
                tracing::info!("rio -> better_pty: create_pty_with_fork");
                ...
                tracing::info!("rio -> better_pty: create_pty_with_spawn");
    ...
    pub fn start(
        ...
        margin: Delta<f32>,
        better_gpu_errors: Option<SugarloafErrors>,
        ...
        if let Some(errors) = better_gpu_errors {
    ...
    pub fn split(&mut self, rich_text_id: usize, split_down: bool) {
        ...
        if self.config.cwd {
            #[cfg(not(target_os = "windows"))]
            {
                ...
                if let Ok(path) = better_pty::foreground_process_path(
...
) -> better_terminal::event::TerminalDamage {
    use better_terminal::crosswords::LineDamage;
    use std::collections::BTreeSet;

    match (old, new) {
        // No selection -> No selection: no damage
        (None, None) => better_terminal::event::TerminalDamage::CursorOnly,

        // Selection cleared or created: need full damage
        (None, Some(_)) | (Some(_), None) => better_terminal::event::TerminalDamage::Full,
        ...
        (Some(old_range), Some(new_range)) => {
            ...
            if old_range.is_block != new_range.is_block {
                return better_terminal::event::TerminalDamage::Full;
            }
            ...
            better_terminal::event::TerminalDamage::Partial(damaged_lines)
        }
    }
}
...
mod selection_damage_tests {
    use super::*;
    use better_terminal::crosswords::pos::{Column, Line, Pos};
    use better_terminal::selection::SelectionRange;
    ...
    fn test_no_selection_to_no_selection() {
        ...
        assert!(matches!(
            damage,
            better_terminal::event::TerminalDamage::CursorOnly
        ));
    }
    ...
    fn test_new_selection_created() {
        ...
        assert!(matches!(damage, better_terminal::event::TerminalDamage::Full));
    }
    ...
    fn test_selection_extend_down() {
        ...
        if let better_terminal::event::TerminalDamage::Partial(lines) = damage {
