...
impl<T: better_terminal::event::EventListener> ContextGridItem<T> {
...
impl<T: better_terminal::event::EventListener> ContextGrid<T> {
    ...
    pub fn update_dimensions(&mut self, better_gpu: &Sugarloaf) {
        for context in self.inner.values_mut() {
            let layout = better_gpu.rich_text_layout(&context.val.rich_text_id);
