use better_terminal::config::window::{Decorations, WindowMode};
use better_terminal::config::Config;
use better_window::window::{
...
pub fn create_window_builder(
    ...
    let mut window_builder = WindowAttributes::default()
        .with_title(title)
        .with_min_inner_size(better_window::dpi::LogicalSize {
    ...
    match config.window.decorations {
        ...
        Decorations::Transparent => {
            #[cfg(target_os = "macos")]
            {
                use better_window::platform::macos::WindowAttributesExtMacOS;
    ...
    {
        use better_window::platform::x11::WindowAttributesExtX11;
        window_builder =
    ...
    {
        use better_window::platform::wayland::WindowAttributesExtWayland;
        window_builder =
            window_builder.with_name(APPLICATION_ID.to_lowercase(), APPLICATION_ID);
    }
    ...
    {
        use better_window::platform::windows::WindowAttributesExtWindows;
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        window_builder = window_builder
            .with_colorspace(config.window.colorspace.to_better_window_colorspace());
    ...
    match config.window.mode {
        ...
        _ => {
            window_builder =
                window_builder.with_inner_size(better_window::dpi::LogicalSize {
...
pub fn configure_window(winit_window: &Window, config: &Config) {
    ...
    // winit_window.set_ime_cursor_area(better_window::dpi::PhysicalPosition::new(500.0, 500.0), better_window::dpi::LogicalSize::new(400, 400));
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        use better_window::platform::macos::{OptionAsAlt, WindowExtMacOS};
    ...
    #[cfg(target_os = "windows")]
    {
        use better_terminal::config::window::WindowsCornerPreference;
        use better_window::platform::windows::WindowExtWindows;
        ...
        if let Some(with_corner_preference) = &config.window.windows_corner_preference {
            let preference = match with_corner_preference {
                WindowsCornerPreference::Default => {
                    better_window::platform::windows::CornerPreference::Default
                }
                WindowsCornerPreference::DoNotRound => {
                    better_window::platform::windows::CornerPreference::DoNotRound
                }
                WindowsCornerPreference::Round => {
                    better_window::platform::windows::CornerPreference::Round
                }
                WindowsCornerPreference::RoundSmall => {
                    better_window::platform::windows::CornerPreference::RoundSmall
                }
            };
