...
use better_terminal::config::Config as RioConfig;
use better_terminal::error::{RioError, RioErrorLevel, RioErrorType};

use better_window::event_loop::ActiveEventLoop;
use better_window::keyboard::{Key, NamedKey};
#[cfg(not(any(target_os = "macos", windows)))]
use better_window::platform::startup_notify::{
    self, EventLoopExtStartupNotify, WindowAttributesExtStartupNotify,
};
use better_window::window::{Window, WindowId};
...
impl Route<'_> {
    ...
    pub fn begin_render(&mut self) {
        ...
        //     Option<HashMap<better_window::window::WindowId, (u64, std::time::Instant)>>,
        // > = Mutex::new(None);
    ...
    pub fn update_config(
        &mut self,
        config: &RioConfig,
        db: &better_terminal::better_gpu::font::FontLibrary,
    ...
    #[inline]
    pub fn has_key_wait(&mut self, key_event: &better_window::event::KeyEvent) -> bool {
        ...
        if self.path == RoutePath::Welcome && is_enter {
            better_terminal::config::create_config_file(None);
            self.path = RoutePath::Terminal;
        }
...
pub struct Router<'a> {
    ...
    pub font_library: Box<better_terminal::better_gpu::font::FontLibrary>,
...
impl Router<'_> {
    pub fn new<'b>(
        fonts: better_terminal::better_gpu::font::SugarloafFonts,
        clipboard: Clipboard,
    ) -> Router<'b> {
        let (font_library, fonts_not_found) =
            better_terminal::better_gpu::font::FontLibrary::new(fonts);
    ...
    pub fn open_config_window(
        ...
        args.push(
            better_terminal::config::config_file_path()
                .display()
                .to_string(),
        );
        let new_config = RioConfig {
            shell: better_terminal::config::Shell {
    ...
    pub fn create_window<'a>(
        ...
        event_proxy: EventProxy,
        config: &'a better_terminal::config::Config,
...
impl<'a> RouteWindow<'a> {
    pub fn configure_window(&mut self, config: &better_terminal::config::Config) {
