...
use better_terminal::clipboard::{Clipboard, ClipboardType};
use better_terminal::config::colors::ColorRgb;
use better_window::application::ApplicationHandler;
use better_window::event::{
    ElementState, Ime, MouseButton, MouseScrollDelta, StartCause, TouchPhase, WindowEvent,
};
use better_window::event_loop::ActiveEventLoop;
use better_window::event_loop::ControlFlow;
use better_window::event_loop::{DeviceEvents, EventLoop};
#[cfg(target_os = "macos")]
use better_window::platform::macos::ActiveEventLoopExtMacOS;
#[cfg(target_os = "macos")]
use better_window::platform::macos::WindowExtMacOS;
use better_window::window::WindowId;
use better_window::window::{CursorIcon, Fullscreen};
use std::error::Error;
...
impl Application<'_> {
    pub fn new<'app>(
        config: better_terminal::config::Config,
        config_error: Option<better_terminal::config::ConfigError>,
        ...
        let _ = configuration_file_updates(
            better_terminal::config::config_dir_path(),
            event_proxy.clone(),
        );
...
impl ApplicationHandler<EventPayload> for Application<'_> {
        ...
        if !self.scheduler.scheduled(timer_id) {
            self.scheduler.schedule(
                EventPayload::new(RioEventType::Rio(RioEvent::UpdateTitles), unsafe {
                    better_window::window::WindowId::dummy()
                }),
    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: EventPayload) {
        ...
        match event.payload {
            ...
            RioEventType::Rio(RioEvent::UpdateGraphics { route_id, queues }) => {
                if let Some(route) = self.router.routes.get_mut(&window_id) {
                    // Process graphics directly in better_gpu
                    let better_gpu = &mut route.window.screen.better_gpu;

                    for graphic_data in queues.pending {
                        better_gpu.graphics.insert(graphic_data);
                    }

                    for graphic_data in queues.remove_queue {
                        better_gpu.graphics.remove(&graphic_data);
                    }
            ...
            RioEventType::Rio(RioEvent::UpdateConfig) => {
                let (config, config_error) = match better_terminal::config::Config::try_load()
                {
                    Ok(config) => (config, None),
                    Err(error) => (better_terminal::config::Config::default(), Some(error)),
                };

                let has_font_updates = self.config.fonts != config.fonts;

                let font_library_errors = if has_font_updates {
                    let new_font_library = better_terminal::better_gpu::font::FontLibrary::new(
                ...
                if let Some(route) = self.router.routes.get_mut(&window_id) {
                    if route_id == route.window.screen.ctx().current_route() {
                        ...
                        route
                            ...
                            .renderable_content
                            .pending_update
                            .set_ui_damage(better_terminal::event::TerminalDamage::Partial(
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                let layout = route.window.screen.better_gpu.window_size();
            ...
            WindowEvent::MouseWheel { delta, phase, .. } => {
                ...
                match delta {
                    MouseScrollDelta::LineDelta(columns, lines) => {
                        let layout = route.window.screen.better_gpu.rich_text_layout(&0);
    ...
    fn hook_event(
        ...
        modifiers: &better_window::event::Modifiers,
    ) {
