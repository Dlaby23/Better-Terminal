...
use better_terminal::config::colors::term::TermColors;
use better_terminal::config::colors::{
    term::{List, DIM_FACTOR},
    AnsiColor, ColorArray, Colors, NamedColor,
};
use better_terminal::config::Config;
use better_terminal::event::EventProxy;
...
pub struct Renderer {
    ...
    font_context: better_terminal::better_gpu::font::FontLibrary,
...
impl Renderer {
    ...
    fn is_position_in_hint_matches(
        matches: &[better_terminal::crosswords::search::Match],
        pos: Pos,
    ) -> bool {
    ...
    fn create_line(
        ...
        hint_matches: Option<&[better_terminal::crosswords::search::Match]>,
        ...
        for column in 0..columns {
            ...
            if let Some(hint_label) = self.find_hint_label_at_position(
                ...
                use better_terminal::better_gpu::font_introspector::{Attributes, Weight};
    ...
    pub fn run(
        ...
        if has_search && self.search.rich_text_id.is_none() {
            let search_rich_text = better_gpu.create_temp_rich_text();
            better_gpu.set_rich_text_font_size(&search_rich_text, 12.0);
        ...
        self.update_search_rich_text(better_gpu.content());

        let window_size = better_gpu.window_size();
        let scale_factor = better_gpu.scale_factor();
        ...
        better_gpu.set_visual_bell_overlay(bell_overlay);
...
mod tests {
    ...
    fn test_empty_hint_matches() {
        let matches: Vec<better_terminal::crosswords::search::Match> = vec![];
