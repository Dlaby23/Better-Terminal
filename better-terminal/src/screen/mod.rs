...
use better_terminal::clipboard::Clipboard;
use better_terminal::clipboard::ClipboardType;
use better_terminal::config::renderer::{
    Backend as RendererBackend, Performance as RendererPerformance,
};
use better_terminal::crosswords::pos::{Boundary, CursorState, Direction, Line};
use better_terminal::crosswords::search::RegexSearch;
use better_terminal::event::{ClickState, EventProxy, SearchState};
use better_terminal::better_gpu::{
    layout::RootStyle, Sugarloaf, SugarloafErrors, SugarloafRenderer, SugarloafWindow,
    SugarloafWindowSize,
};
use better_window::event::ElementState;
use better_window::event::Modifiers;
use better_window::event::MouseButton;
#[cfg(target_os = "macos")]
use better_window::keyboard::ModifiersKeyState;
use better_window::keyboard::{Key, KeyLocation, ModifiersState, NamedKey};
use better_window::platform::modifier_supplement::KeyEventExtModifierSupplement;
...
pub struct Screen<'screen> {
    ...
    pub better_gpu: Sugarloaf<'screen>,
    ...
    hints_config: Vec<std::rc::Rc<better_terminal::config::hints::Hint>>,
}

pub struct ScreenWindowProperties {
    pub size: better_window::dpi::PhysicalSize<u32>,
    pub scale: f64,
    pub raw_window_handle: RawWindowHandle,
    pub raw_display_handle: RawDisplayHandle,
    pub window_id: better_window::window::WindowId,
}

impl Screen<'_> {
    pub fn new<'screen>(
        window_properties: ScreenWindowProperties,
        config: &better_terminal::config::Config,
        event_proxy: EventProxy,
        font_library: &better_terminal::better_gpu::font::FontLibrary,
        ...
        let better_gpu_layout =
            RootStyle::new(scale as f32, config.fonts.size, config.line_height);

        let mut better_gpu_errors: Option<SugarloafErrors> = None;

        let better_gpu_window = SugarloafWindow {
        ...
        let better_gpu_renderer = SugarloafRenderer {
            power_preference,
            backend,
            font_features: config.fonts.features.clone(),
            colorspace: config.window.colorspace.to_better_gpu_colorspace(),
        };

        let mut better_gpu: Sugarloaf = match Sugarloaf::new(
            better_gpu_window,
            better_gpu_renderer,
            font_library,
            better_gpu_layout,
            ...
            Err(instance_with_errors) => {
                better_gpu_errors = Some(instance_with_errors.errors);
                instance_with_errors.instance
            }
        };

        better_gpu.update_filters(config.renderer.filters.as_slice());
        ...
        let rich_text_id = better_gpu.create_rich_text();
        ...
        let context_dimension = ContextDimension::build(
            size.width as f32,
            size.height as f32,
            better_gpu.get_rich_text_dimensions(&rich_text_id),
            config.line_height,
            margin,
        );
        ...
        if cfg!(target_os = "macos") {
            better_gpu.set_background_color(None);
        } else {
            better_gpu.set_background_color(Some(renderer.dynamic_background.1));
        }

        if let Some(image) = &config.window.background_image {
            better_gpu.set_background_image(image);
        }
        better_gpu.render();
    ...
    pub fn mouse_position(&self, display_offset: usize) -> Pos {
        ...
        let style = self.better_gpu.style();
    ...
    pub fn update_config(
        ...
        if should_update_font_library {
            self.better_gpu.update_font(font_library);
        }
        let s = self.better_gpu.style_mut();
        ...
        if cfg!(target_os = "macos") {
            self.better_gpu.set_background_color(None);
        } else {
            self.better_gpu
        ...
        if let Some(image) = &config.window.background_image {
            self.better_gpu.set_background_image(image);
        }
    ...
    pub fn change_font_size(&mut self, action: FontSizeAction) {
        ...
        self.better_gpu.set_rich_text_font_size_based_on_action(
        ...
        self.context_manager
            .current_grid_mut()
            .update_dimensions(&self.better_gpu);
    ...
    #[inline]
    pub fn resize(&mut self, new_size: better_window::dpi::PhysicalSize<u32>) -> &mut Self {
        if self
            .context_manager
            .current()
            .renderable_content
        ...
        self.better_gpu.resize(new_size.width, new_size.height);
    ...
    pub fn set_scale(
        &mut self,
        new_scale: f32,
        new_size: better_window::dpi::PhysicalSize<u32>,
    ) -> &mut Self {
        self.better_gpu.rescale(new_scale);
        self.better_gpu.resize(new_size.width, new_size.height);
        self.render();
        self.resize_all_contexts();
        self.context_manager
    ...
    pub fn resize_all_contexts(&mut self) {
        ...
        // the wakeup from pty it will also trigger a better_gpu.render()
    ...
    #[inline]
    pub fn process_key_event(&mut self, key: &better_window::event::KeyEvent) {
        ...
        if self.hint_state.is_active() {
            ...
            match key.logical_key {
                better_window::keyboard::Key::Named(
                    better_window::keyboard::NamedKey::Escape,
                ) => {
                ...
                better_window::keyboard::Key::Named(
                    better_window::keyboard::NamedKey::Backspace,
                ) => {
    ...
    fn should_build_sequence(
        key: &better_window::event::KeyEvent,
        text: &str,
        mode: Mode,
        mods: ModifiersState,
    ) -> bool {
    ...
    pub fn split_right_with_config(&mut self, config: better_terminal::config::Config) {
        let rich_text_id = self.better_gpu.create_rich_text();
        self.context_manager
    ...
    fn alt_send_esc(&mut self, key: &better_window::event::KeyEvent, text: &str) -> bool {
    ...
    pub fn update_highlighted_hints(&mut self) -> bool {
        ...
        if let Some(hint_match) = highlighted_hint {
            // Mark the hint range as damaged so it gets re-rendered
            {
                ...
                let hint_range = better_terminal::selection::SelectionRange::new(
                    hint_match.start,
                    hint_match.end,
                    false,
                );
    ...
    fn find_hint_at_point(
        &self,
        terminal: &better_terminal::crosswords::Crosswords<EventProxy>,
        point: better_terminal::crosswords::pos::Pos,
        _modifiers: better_window::keyboard::ModifiersState,
    ...
    fn find_hyperlink_at_point(
        ...
        if let Some(hyperlink) = cell.hyperlink() {
            ...
            while start_col > better_terminal::crosswords::pos::Column(0) {
            ...
            let hint_config = std::rc::Rc::new(better_terminal::config::hints::Hint {
                regex: None,
                hyperlinks: true,
                post_processing: true,
                persist: false,
                action: better_terminal::config::hints::HintAction::Command {
                    command: better_terminal::config::hints::HintCommand::Simple(
                        "xdg-open".to_string(),
                    ),
                },
                mouse: better_terminal::config::hints::HintMouse::default(),
                binding: None,
            });
            ...
            return Some(crate::hints::HintMatch {
                text: uri,
                start: better_terminal::crosswords::pos::Pos::new(point.row, start_col),
                end: better_terminal::crosswords::pos::Pos::new(point.row, end_col),
                hint: hint_config,
            });
        }
    ...
    fn find_regex_match_at_point(
        ...
        point: better_terminal::crosswords::pos::Pos,
        regex: &regex::Regex,
        hint_config: std::rc::Rc<better_terminal::config::hints::Hint>,
    ) -> Option<crate::hints::HintMatch> {
        let grid = &terminal.grid;
        ...
        for col in 0..grid.columns() {
            let cell = &grid[point.row][better_terminal::crosswords::pos::Column(col)];
        ...
        for mat in regex.find_iter(line_text) {
            let start_col = better_terminal::crosswords::pos::Column(mat.start());
            let end_col =
                better_terminal::crosswords::pos::Column(mat.end().saturating_sub(1));
            ...
            if point.col >= start_col && point.col <= end_col {
                ...
                let (processed_start, processed_end) = if hint_config.post_processing {
                    self.hint_post_processing(
                        ...
                        start_col,
                        end_col,
                        better_terminal::crosswords::pos::Line(point.row.0),
                    )
                ...
                if hint_config.post_processing {
                    ...
                    for col in processed_start.0..=processed_end.0 {
                        let cell =
                            &grid[point.row][better_terminal::crosswords::pos::Column(col)];
                ...
                return Some(crate::hints::HintMatch {
                    text: match_text,
                    start: better_terminal::crosswords::pos::Pos::new(
                        point.row,
                        processed_start,
                    ),
                    end: better_terminal::crosswords::pos::Pos::new(point.row, processed_end),
                    hint: hint_config,
                });
            }
        }
        ...
        S: AsRef<OsStr>,
    {
        #[cfg(unix)]
        {
            ...
            match better_pty::spawn_daemon(program, args, main_fd, *shell_pid) {
        ...
        #[cfg(windows)]
        {
            match better_pty::spawn_daemon(program, args) {
                Ok(_) => tracing::debug!("Launched {} with args {:?}", program, args),
                Err(_) => {
                    tracing::warn!("Unable to launch {} with args {:?}", program, args)
                }
        ...
        if is_search_active {
            ...
            // Force invalidation for search with full damage
            {
                ...
                current
                    .renderable_content
                    .pending_update
                    .set_ui_damage(better_terminal::event::TerminalDamage::Full);
            }
        }
    ...
    pub fn update_ime_cursor_position_if_needed(
        &mut self,
        window: &better_window::window::Window,
    ) {
        ...
        window.set_ime_cursor_area(
            better_window::dpi::PhysicalPosition::new(pixel_x as f64, pixel_y as f64),
            better_window::dpi::PhysicalSize::new(cell_width as f64, cell_height as f64),
        );
    }
    ...
    pub fn start_hint_mode(
        &mut self,
        hint: std::rc::Rc<better_terminal::config::hints::Hint>,
    ) {
    ...
    fn execute_hint_action(&mut self, hint_match: &crate::hints::HintMatch) {
        use better_terminal::config::hints::{HintAction, HintCommand, HintInternalAction};
        ...
        match &hint_match.hint.action {
            HintAction::Action { action } => match action {
                ...
                HintInternalAction::Select => {
                    ...
                    let selection = better_terminal::selection::SelectionRange::new(
                        hint_match.start,
                        hint_match.end,
    ...
    pub fn update_hint_state(&mut self) {
        use better_terminal::event::TerminalDamage;
        ...
        if self.hint_state.is_active() {
            ...
            let matches: Vec<better_terminal::crosswords::search::Match> = self
                .hint_state
                .matches()
                .iter()
                    ...
                    for label in hint_labels {
                        ...
                        if line >= 0 && (line as usize) < screen_lines {
                            damaged_lines.insert(
                                better_terminal::crosswords::LineDamage::new(
                                    line as usize,
                                    true,
                                ),
                            );
            ...
            for (match_index, remaining_label) in visible_labels {
                if let Some(hint_match) = matches.get(match_index) {
                    ...
                    for (char_index, &label_char) in remaining_label.iter().enumerate() {
                        let position = better_terminal::crosswords::pos::Pos::new(
    ...
    fn hint_post_processing(
        &self,
        terminal: &better_terminal::crosswords::Crosswords<EventProxy>,
        start_col: better_terminal::crosswords::pos::Column,
        end_col: better_terminal::crosswords::pos::Column,
        row: better_terminal::crosswords::pos::Line,
    ) -> Option<(
        better_terminal::crosswords::pos::Column,
        better_terminal::crosswords::pos::Column,
    )> {
        use better_terminal::crosswords::grid::BidirectionalIterator;

        let grid = &terminal.grid;
        let start_pos = better_terminal::crosswords::pos::Pos::new(row, start_col);
        let end_pos = better_terminal::crosswords::pos::Pos::new(row, end_col);
