use crate::better_gpu::{Colorspace, SugarloafWindow, SugarloafWindowSize};
...
impl Context<'_> {
    pub fn new<'a>(
        better_gpu_window: SugarloafWindow,
        ...
        let size = better_gpu_window.size;
        let scale = better_gpu_window.scale;

        let surface: wgpu::Surface<'a> =
            instance.create_surface(better_gpu_window).unwrap();
